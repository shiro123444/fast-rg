...
class MoeMailMailbox(BaseMailbox):
    """MoeMail 邮箱服务 - 支持 API Key 直连模式和旧版注册/登录模式"""
    ...
    def __init__(
        ...
        self._session = None
        self._email = None
        # 当提供了 api_key 时，使用直连模式（无需注册/登录）
        self._direct_mode = bool(self.api_key)
    ...
    def _register_and_login(self) -> str:
        """旧版模式：注册 MoeMail 账号并登录获取 session"""
    ...
    def _get_available_domains(self) -> list:
        ...
        try:
            # 新版 API：/api/emails/domains
            r = self._session.get(
                f"{self.api}/api/emails/domains",
                headers=self._api_headers(),
                timeout=10,
            )
            if r.status_code == 200:
                data = r.json()
                # 可能直接返回列表，也可能在 domains 字段中
                if isinstance(data, list):
                    domains = [str(d).strip() for d in data if d]
    ...
    def get_email(self) -> MailboxAccount:
        ...
        domain = random.choice(domains) if domains else "moemail.app"
        ...
        if r.status_code >= 400:
            raise RuntimeError(
                f"MoeMail 生成邮箱失败: HTTP {r.status_code} {r.text[:200]}"
            )
        ...
        if not self._email:
            raise RuntimeError(f"MoeMail 生成邮箱返回空地址: {data}")
        if not email_id:
            print(f"[MoeMail] 警告: 生成邮箱缺少 id: {data}")
        if email_id:
    ...
    def _fetch_message_detail(self, email_id: str, message_id: str) -> str:
        """获取单封邮件的详细内容"""
        self._ensure_session()
        try:
            r = self._session.get(
                f"{self.api}/api/emails/{email_id}/{message_id}",
                headers=self._api_headers(),
                timeout=10,
            )
            if r.status_code == 200:
        ...
        **kwargs,
        ...
        def poll_once() -> Optional[str]:
            ...
            try:
                ...
                if _poll_count[0] <= 3 or _poll_count[0] % 5 == 0:
                    self._log(
                        f"[MoeMail] 轮询 #{_poll_count[0]}: "
                ...
                if not msgs and isinstance(data, list):
                    msgs = data
                # 按 id 或时间倒序，优先处理最新邮件
                msgs = sorted(msgs, key=lambda x: x.get("id", 0), reverse=True)
                for msg in msgs:
                    mid = str(msg.get("id", ""))
                    if not mid or mid in seen:
                        continue

                    # 时间过滤：跳过 OTP 发送之前的旧邮件
                    if otp_cutoff:
                        ...
                        if created_at:
                            try:
                                ...
                                for fmt in ("%Y-%m-%dT%H:%M:%S.%fZ", "%Y-%m-%dT%H:%M:%SZ", "%Y-%m-%d %H:%M:%S"):
                                    try:
                                        mail_ts = datetime.strptime(created_at, fmt).replace(tzinfo=timezone.utc).timestamp()
                                        break
                                    except ValueError:
                                        continue
                                else:
                                ...
                                if mail_ts and mail_ts < otp_cutoff:
                                    self._log(f"[MoeMail] 跳过旧邮件 id={mid} created_at={created_at}")
                                    continue
                            except Exception:
                                pass
                    ...
                    # 如果列表数据太少，尝试获取邮件详情
                    if len(body.strip()) < 20 and mid:
                        detail = self._fetch_message_detail(
                            account.account_id, mid
                        )
                        if detail:
                            body = detail
                    ...
                    # 跳过已使用过的验证码
                    if code and code in exclude_codes:
                        self._log(f"[MoeMail] 跳过已用验证码 id={mid} code={code}")
                        continue

                    if code:
                        self._log(f"[MoeMail] 命中新验证码 id={mid} code={code}")
                        return code
            except Exception as e:
                self._log(f"[MoeMail] 轮询异常: {e}")
            return None
...
class LuckMailMailbox(BaseMailbox):
    ...
    def __init__(
        ...
        self._variant_mode = variant_mode or None
        self._order_no = None
        self._token = None
        self._email = None
    ...
    def get_email(self) -> MailboxAccount:
        ...
        try:
            ...
            if self._variant_mode:
                body["variant_mode"] = self._variant_mode
